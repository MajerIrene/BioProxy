...
where $\alpha$ is a tuning hyperparameter which decides the cost complexity of the tree and $T$ is the number of terminal leaf of a specific tree, in this way we can use cross-validation to find the optimal value of $\alpha$. This formulation is indeed very similar to the Lasso regularization discussed before and in the same way if $\alpha = 0$ the tree is the unpruned tree. Let's now fit a regression tree and look for the optimal value of $\alpha$, here called ***cp*** which stands for cost complexity.
